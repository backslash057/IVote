...
CREATE TABLE IF NOT EXISTS Campaign (
  ...
  price_per_vote INT NOT NULL DEFAULT 100,
...
-- Initial demo data. Temporary password:
...
--   UNION ALL SELECT 'Nadia Eteme', 1, 24, 'Tech', 'Plateforme de santé numérique accessible.', 'Entrepreneure spécialisée dans les solutions numériques inclusives.', 'Tech', 'AfroTech Pitch Awards 2026'
...
-- SELECT 
--   CONCAT('TXN-SEED-', candidate.candidate_id, '-', seed.vote_count) AS transaction_ref,
--   candidate.candidate_id,
--   candidate.campaign_id,
--   seed.payment_method,
--   seed.vote_count,
--   (seed.vote_count * campaign.price_per_vote) AS amount_fcfa
-- FROM (
--   SELECT 'Amina Diallo' AS name, 'mtn_momo' AS payment_method, 14820 AS vote_count
--   UNION ALL SELECT 'Koffi Mensah', 'orange_money', 12450
--   UNION ALL SELECT 'Grace Nkem', 'mtn_momo', 9340
--   UNION ALL SELECT 'Yannick Tcham', 'orange_money', 6730
--   UNION ALL SELECT 'Sara Benali', 'mtn_momo', 4980
--   UNION ALL SELECT 'David Osei', 'orange_money', 4120
--   UNION ALL SELECT 'Nadia Eteme', 'mtn_momo', 12000
--   UNION ALL SELECT 'Moussa Traore', 'orange_money', 9500
--   UNION ALL SELECT 'Chisom Okafor', 'mtn_momo', 6910
--   UNION ALL SELECT 'Aya Mbaye', 'orange_money', 28000
--   UNION ALL SELECT 'Jean Kondo', 'mtn_momo', 24000
--   UNION ALL SELECT 'Lina Sarr', 'orange_money', 19200
-- ) AS seed
-- JOIN Candidate candidate ON candidate.name = seed.name
-- JOIN Campaign campaign ON campaign.campaign_id = candidate.campaign_id
-- WHERE NOT EXISTS (
