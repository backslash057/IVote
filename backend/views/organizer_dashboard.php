...
<head>
    ...
    <title>Tableau de Bord - <?= htmlspecialchars($activeCampaign['title']) ?></title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen pb-28 pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans">
    ...
    <header class="fixed top-0 left-0 right-0 z-40 bg-slate-950/90 backdrop-blur-xl border-b border-slate-800 transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
            <div class="flex items-center gap-3 sm:gap-6">
                <a href="/organizer-campaigns.php" class="flex items-center gap-2.5 text-left group cursor-pointer">
                    ...
                    <div>
                        <span class="text-base font-bold tracking-tight text-white flex items-center gap-1.5">IVote</span>
                        <span class="text-[10px] text-slate-400 hidden sm:block">Vote & Paiement Mobile Money</span>
                    </div>
                </a>
            </div>
            <div class="flex items-center gap-2">
                <a href="/organizer-campaigns.php" class="text-xs font-semibold text-slate-400 hover:text-emerald-400 transition-colors">
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-1">
                ...
                <span>Tableau de Bord • <?= htmlspecialchars($userSession['organizationName'] ?? $activeCampaign['organization']) ?></span>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            <a href="/public-page" target="_blank" class="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 text-xs font-semibold border border-slate-800 transition-colors">
                 Page Publique ↗
            </a>
    ...
    <!-- 2. STATISTIC CARDS (Placées avant la navigation des onglets) -->
    ...
    <!-- TAB 1: CANDIDATES (ACTIF PAR DÉFAUT) -->
    <div id="tab-candidates" class="tab-content rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl overflow-hidden">
    ...
    <div id="tab-transactions" class="tab-content hidden rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl p-4 sm:p-6 space-y-4">
        <div class="flex flex-col sm:flex-row gap-3">
            <input type="text" id="txSearch" onkeyup="filterTransactions()" placeholder="Rechercher par réf, candidat..." class="flex-1 px-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-500 focus:border-emerald-500 outline-none">
            <select id="paymentFilter" onchange="filterTransactions()" aria-label="Filtrer par moyen de paiement" class="px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-300 outline-none">
    ...
    <div id="tab-payouts" class="tab-content hidden rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl p-4 sm:p-6 space-y-4">
        <div class="flex items-center justify-between pb-4 border-b border-slate-800">
            ...
            <button onclick="toggleModal('payoutModal', true)" class="px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-500 transition">Demander un Retrait</button>
        </div>
        <div class="overflow-x-auto rounded-2xl border border-slate-800">
            <table class="w-full text-left text-xs sm:text-sm">
                <thead class="bg-slate-950/80 text-slate-400 uppercase text-[10px]">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md space-y-4">
            ...
            <form method="POST" action="/request-payout" class="space-y-3">
                <div>
                    ...
                    <input type="number" name="amount" max="<?= $availableBalance ?>" required class="w-full mt-1 p-2 bg-slate-950 border border-slate-800 rounded-xl text-white text-sm outline-none focus:border-emerald-500">
                </div>
                <div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" onclick="toggleModal('payoutModal', false)" class="px-4 py-2 bg-slate-800 text-slate-300 text-xs rounded-xl hover:bg-slate-700">Annuler</button>
                    <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-xs font-bold rounded-xl hover:bg-emerald-500">Confirmer</button>
                </div>
            </form>
        </div>
    </div>
